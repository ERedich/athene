...
export function assetTypeDisplayLabel(
  ...
  return fallback(`assets.types.${type}`);
}

export function assetTypeColorHex(type: AssetTypeSlug, cfg: AssetTypeDisplayConfig | null): string {
  return cfg?.[type]?.colorHex?.trim() || DEFAULT_ASSET_TYPE_DISPLAY_CONFIG[type].colorHex;
}

/** 10% opacity tint for GN-CATR row backgrounds. */
export function assetTypeRowBackground(type: AssetTypeSlug, cfg: AssetTypeDisplayConfig | null): string {
  const hex = assetTypeColorHex(type, cfg);
