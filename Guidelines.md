...
- **Exception — Getting Started**: the **Getting Started / Erste Schritte** app ([`frontend/src/pages/GettingStartedPage.tsx`](frontend/src/pages/GettingStartedPage.tsx)) is a guided content hub (guide tiles + step lists with deep links) — not a `DataTable`. CRUD context menu, row double-click, header search, and `setHeaderRowCount` do **not** apply.
- **Exception — Customize menu / Menü anpassen**: the **Customize menu** app ([`frontend/src/pages/CustomizeMenuPage.tsx`](frontend/src/pages/CustomizeMenuPage.tsx), editor [`CustomizeMenuEditorPage.tsx`](frontend/src/pages/CustomizeMenuEditorPage.tsx)) manages named **menu configurations** (`navMenuConfig`) with separate **web** and **mobile** layouts — not a primary CRUD `DataTable`. Hub list + nested drag editors; New/Edit/Delete for configs and for **custom** nav entries only. Catalog/system-default entries are reorder/hide/move only (not rename/delete), including **Menü anpassen** itself. Mobile layout is **flat parent items only** (no groups/sub-items). Header search and `setHeaderRowCount` apply. Saving a config does **not** assign it to the current user; user assignment is done in the **Zuweisungen** app (`users.navMenuConfigId`).
- **Exception — Zuweisungen / Assignments**: the **Zuweisungen** app ([`frontend/src/pages/AssignmentsPage.tsx`](frontend/src/pages/AssignmentsPage.tsx), type workspace [`AssignmentsTypePage.tsx`](frontend/src/pages/AssignmentsTypePage.tsx), user detail [`AssignmentsUserPage.tsx`](frontend/src/pages/AssignmentsUserPage.tsx)) is the central place to link users to menus (`navMenuConfig`, exclusive), search presets (`workOrderSearchPresetShare`, share — including presets owned by others), and **permission templates** (`users.permissionTemplateId`, exclusive last-applied marker + **copy-on-apply** into `userPermission`). Hub uses type tiles (not a DataTable): no CRUD context menu and no `setHeaderRowCount` on the hub (same class as Getting Started). Dashboard and layout tiles are visible but disabled until those assignment models exist. Type workspace: record + user tables; context menu is **Zuweisen** + **In Quell-App öffnen** only (not New/Edit/Delete). Header search and `setHeaderRowCount` apply on the workspace (active view’s filtered row count). Bulk site targets use **`workingSiteId` only** (not `userSite` membership). Unassigning a menu sets `navMenuConfigId` to null (catalog default sidebar). Unassigning a permission template clears `permissionTemplateId` only (grants stay). Assigning a template requires **`permissions.manage`** (in addition to `assignments.update`) and replaces the target’s `userPermission` with the template grants. Athene must not write assignments; the existing exception that Athene may **create** search presets for the current user remains, and Athene still must not share presets.
- **Exception — Systemwerkzeuge / System tools**: the **Systemwerkzeuge** app ([`frontend/src/pages/SystemToolsPage.tsx`](frontend/src/pages/SystemToolsPage.tsx), procedure [`SystemToolsProcedurePage.tsx`](frontend/src/pages/SystemToolsProcedurePage.tsx)) is a hub of manual system procedures (tiles, not a DataTable). Hub: header search filters procedure tiles; no CRUD context menu and no `setHeaderRowCount`. Disabled tiles (e.g. BANF create) stay visible with “Folgt / Coming soon”. Procedure pages show KPIs + confirm-then-run actions only (no DataTable, no header search, no row count). First enabled procedure rolls out due maintenance plans via existing `POST /api/maintenance-plans/generate-due` (same filter as the daily sweep); the Wartungspläne header shortcut **Fällige erzeugen** remains.
- **Exception — Berechtigungswesen / Permissions admin**: the **Berechtigungswesen** app ([`frontend/src/pages/PermissionsAdminPage.tsx`](frontend/src/pages/PermissionsAdminPage.tsx), route `/berechtigungswesen`) is a standalone Administration app with page tabs: **Benutzerrechte** (user list + BMW matrix / template fill via `PUT /api/users/:id/permissions`, requires `permissions.manage`) and **Vorlagen** (CRUD `DataTable` of `permissionTemplate` + grants; editor BMW with Stammdaten | matrix; **Benutzern zuweisen** deep-links into Zuweisungen). Templates are **copy-on-apply** onto `userPermission` (not a live role link); bulk/user assignment of templates is done in **Zuweisungen** (`users.permissionTemplateId` tracks last applied). Catalog `appKey` remains `permission-templates`. Meta keys (`permissions.manage`, `permission-templates.*`, `layout-editor.editSystem`) are not part of the operational seed except for the seed admin.
- **Users dialog**: create/edit is Stammdaten only (MW). New users without an explicit `permissionKeys` payload receive the operational permission set server-side (same as migration grant-all). User grants are edited only in **Berechtigungswesen**.
- **Permissions model**: Catalog in [`backend/src/permissionCatalog.ts`](backend/src/permissionCatalog.ts) (`{appKey}.{action}`). Grants stored per user in `userPermission`. Site access (`userSite` / `workingSiteId`), menu assignment (`navMenuConfig`), and permission-template assignment (`permissionTemplateId`) remain separate axes. APIs enforce via `requireAppCrud` / `requirePermission` (never trust the menu). UI hides header/context-menu actions and nav items without the matching right; deep links without `{app}.view` show a forbidden page. Shared work-order process actions use virtual keys `workOrder.start|pause|cancel|complete|feedback|assign|subscribe` for both Monitoring and Aufträge. Do **not** use `loginName === "admin"` as a permission check — use `layout-editor.editSystem` / `permissions.manage` instead. New catalog keys are deny-by-default until granted.
- **Search (mandatory per app)**: every main-window **app** that shows a primary **DataTable** or equivalent **scrollable list** must expose a **header search** field. That includes **App parameters** (tabbed tables), **Sites**, **Users**, **Cost centers**, **Assets**, **Audit log**, **Translations / Übersetzungen**, and any future list apps. **Dashboard** and other placeholder-only views are exempt until they ship a list or table.
  ...
  - **Styling**: use the shared class **`app-header-search-input`** on `InputText` (padding/overrides live in [`frontend/src/index.css`](frontend/src/index.css)); do not rely only on utility padding because global `.p-inputtext` rules use `!important`. Add **`h-9 w-56 !rounded-sm text-sm`** on header search fields. CRUD and view-toggle header actions use **`app-header-action-nav-item`** from [`frontend/src/lib/headerActionClasses.ts`](frontend/src/lib/headerActionClasses.ts) (`h-9`, text label); **Neu** adds `--create`, **Löschen** adds `--delete`, active toggles add **`--active`**. Prefer those shared exports over local Tailwind copies. Prime **`app-header-toolbar-btn`** only when a header control must be a Prime `Button`.
...
| **Anweisungen (Instructions)** | Work order has at least one instruction (`todoCount > 0`). Badge shows **open** (unchecked) count; icon stays visible when all are done (badge can be **0**). Click opens a **read-only checklist popover** (not the edit dialog). When **`todoCount = 0`**: same green chrome at **10% opacity**, control **disabled** (`app-ref-button--todos-empty`) — always visible in Monitoring / Aufträge reference columns. | **`green-300`** | `app-ref-button--todos` |
...
- **Disabled / empty (other kinds)**: for **material**, **purchase**, **work orders**, **inspection points**, and **Anweisungen / instructions** (when count = 0), use the matching `app-ref-button--…-empty` class with the control **disabled** — **do not** use the strong green / pink / violet / amber fills on an enabled button. **Documents** when empty: use **`app-ref-button--documents-inactive`** (transparent chrome, bluish icon only — not a filled pill; on Assets the control is disabled). Work orders when empty: use **`app-ref-button--work-orders-empty`**. Inspection points when empty: use **`app-ref-button--inspection-points-empty`** (disabled). Anweisungen when empty: use **`app-ref-button--todos-empty`** (disabled).
...
- Athene knows the current user context: user name, login, working site, accessible sites, linked employee, workgroups / Fachgruppen, and **permissions** (`permissions[]` from `/api/auth/me` / `userPermission`). Athene tools must refuse with `permission_denied` when the user lacks the matching catalog key (e.g. `work-orders.create` for create tools). Athene must never expose or infer password hashes, passwords, API keys, session secrets, or similar sensitive data, and must never change them.
