...
| **Anweisungen (Instructions)** | Work order has at least one instruction (`todoCount > 0`). Badge shows **open** (unchecked) count; icon stays visible when all are done (badge can be **0**). Click opens a **read-only checklist popover** (not the edit dialog). | **`green-300`** | `app-ref-button--todos` |
