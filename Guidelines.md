...
- **Work-order feedback (Rückmeldung)** on web and mobile: each feedback remark field (main remark and pause remark) has a **microphone** control using the same Whisper pipeline. A **Athene** control on the feedback dialog header (web: star icon left of close; mobile: sparkles + close on the quick modal, sparkles in the editor header on the feedback tab) opens Athene in **feedback mode** with draft remark text, asset/work-order context, and suggestion chips for similar asset history and proofreading. Corrections are offered via **Text übernehmen / Apply text** only after the user confirms; Athene never saves feedback automatically. Backend tool: `listWorkOrdersByAsset`. Legacy `POST /api/assistant/localize-spoken-text` (text-only) remains for debugging.
- Athene must not add, change, or delete records for apps in the **Stammdaten / master-data** category, including sites, users, employees, workgroups, cost centers, warehouses, spare parts, classifications, app parameters, translations, table viewer metadata, and search configuration.
- Athene may create work orders under user guidance via assistant tools `createWorkOrder` (explicit UUID references) or `createWorkOrderFromOrder` (copy from an existing Auftragsnummer). Copying must use the template tool so asset/cost-center/workgroup UUIDs are taken from the database, not from displayed business keys. Creation enforces the same required fields, site rules, workgroup rules, classification rules, and reference validations as the normal UI.
- **Work-order copy (web):** In **Monitoring** and **Work orders**, the context menu **Auftrag kopieren / Copy work order** opens a name dialog, then the create dialog prefilled from the template row. On save, `POST /api/work-orders` stores `originalWo` (UUID of the source work order). Normal **Neu / New** leaves `originalWo` null. `originalWo` is immutable after create. Athene `createWorkOrderFromOrder` sets the same field when creating via chat.
- **Athene + Kalendar (planning):** On the **Kalendar** app, right-click a work-order event bar to open Athene with calendar context (visible date range + selected order). Assistant tools: `listWorkOrdersInPlanningWindow`, `analyzeWorkOrderPlanning`, `findPlanningSlots` (one order), **`planSequentialWorkOrderSlots`** (two or more orders on the **same asset** — required before proposing dates), `rescheduleWorkOrdersBatch` (apply a validated multi-order plan after confirmation), `rescheduleWorkOrder` (single order only). Athene must **never invent dates** — only present intervals returned by tools. **Planning collisions** (same **asset**, overlapping intervals) are **warnings**, not hard blocks: ask the user (e.g. *Am gleichen Tag gibt es bereits einen Auftrag für Asset XYZ — trotzdem verschieben?*) and only then save with `allowAssetOverlap: true` on `PUT` / reschedule tools. **Kalendar drag-and-drop** shows the same question in the move confirm panel before **Trotzdem verschieben**. **Not before today:** `plannedStart` must be on or after the current calendar day in **Europe/Berlin**. UI apply: `[ATHENE_APPLY:reschedule:{…}]` or `[ATHENE_APPLY:rescheduleBatch:[{…},…]]` → **Termin übernehmen** / **Alle Termine übernehmen**. Athene must not change status, asset, or master data when rescheduling.
