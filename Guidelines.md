...
- Athene supports **voice input** in the assistant drawer when the platform provides speech recognition (desktop browsers with Web Speech API; mobile web the same; native iOS/Android via device speech recognition in development builds). Requires HTTPS (or localhost) and microphone permission. Dictated text fills the message field; the user still sends with **Senden / Send**.
- **Work-order feedback (Rückmeldung)** on web and mobile: each feedback remark field (main remark and pause remark) has a **microphone** control. Speech is recognized locally, then `POST /api/assistant/localize-spoken-text` returns text in the active UI language (de/en). A **Athene** control on the feedback dialog header (web: star icon left of close; mobile: sparkles + close on the quick modal, sparkles in the editor header on the feedback tab) opens Athene in **feedback mode** with draft remark text, asset/work-order context, and suggestion chips for similar asset history and proofreading. Corrections are offered via **Text übernehmen / Apply text** only after the user confirms; Athene never saves feedback automatically. Backend tool: `listWorkOrdersByAsset`.
